...
const DEFAULT_FONT_NAME = '/Helv';

// Keep a single default font size so the generated PDF text looks uniform
// across fields and overlays, while still allowing a small fit-based shrink.
const BASE_SIZE = 9;
const DEFAULT_TEXT_SIZE = 9;
...
function applyFieldAction(form, font, action, value, answers, question) {
  if (action.text) {
    ...
    ensureDefaultAppearance(field, DEFAULT_TEXT_SIZE);
    ...
    field.setFontSize(DEFAULT_TEXT_SIZE);
    field.setText(text);
    return;
  }
  if (action.comb) {
...
// Normalize every generated form field to a single built-in Helvetica font so
// the output stays visually consistent across the template and our overlays.
function ensureDefaultAppearance(field, size) {
  const appearance = `${DEFAULT_FONT_NAME} ${size} Tf 0 g`;
  try {
...
async function applyDrawAction(doc, font, ink, action, value, answers, question) {
  ...
  if (action.drawX) {
    ...
    if (spec.tat) {
      x = spec.tat[0];
      y = page.getHeight() - spec.tat[1] - DEFAULT_TEXT_SIZE;
    } else {
      [x, y] = spec.at;
    }
    page.drawText('X', { x, y, size: DEFAULT_TEXT_SIZE, font, color: ink });
    return;
  }
  if (action.draw) {
