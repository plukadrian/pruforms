  ...
  if (action.text) {
    ...
    if (!action.multiline) {
      try {
        ...
        // Leave vertical breathing room: pdf-lib pushes text against the top
        // edge when the glyph height nearly fills a short field, so cap the
        // size to ~box height minus padding to keep it centered inside.
        const maxByHeight = rect.height - 3;
        if (maxByHeight < size) size = Math.max(5, maxByHeight);
