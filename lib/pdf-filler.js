...
async function generatePdf(definition, answers) {
  ...
  if (form) {
    ...
    try {
      form.flatten();
      // Some source forms share widgets across fields. Flattening can leave
      // dangling annotation references after the shared objects are removed.
...
function applyFieldAction(form, font, action, value, answers, question) {
  if (action.text) {
    ...
    // Use one consistent font size for every single-line answer across all
    // forms; only shrink when the text is genuinely too wide for its box.
    let size = action.multiline ? action.size || MULTILINE_BASE_SIZE : BASE_SIZE;
    if (!action.multiline) {
      try {
        const widget = field.acroField.getWidgets()[0];
        const rect = widget.getRectangle();
        // pdf-lib jams text against the top edge of a short field; give the
        // box enough vertical room (keeping the same centre so the text still
        // sits on the visible box) so the glyph is centred at full size.
        if (rect.height < 13) {
          const cy = rect.y + rect.height / 2;
          widget.setRectangle({ x: rect.x, y: cy - 6.5, width: rect.width, height: 13 });
        }
        const w = widget.getRectangle().width;
  ...
  if (action.lines) {
    ...
    for (const name of action.lines) {
      const field = form.getTextField(name);
      ensureDefaultAppearance(field, action.size || MULTILINE_BASE_SIZE);
      ...
      field.setFontSize(action.size || MULTILINE_BASE_SIZE);
    }
    return;
  }
  if (action.check) {
